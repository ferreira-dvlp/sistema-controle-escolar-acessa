...
Os arquivos com os dados reais (`js/firebase-config.js` e `js/admin-auth.js`) **não vêm prontos** — ficam de fora do projeto de propósito, pra você nunca correr o risco de subir sua chave ou senha pra algum lugar público (como o GitHub) sem querer. No lugar deles, tem versões de exemplo que você copia:
